...
/**
 * Detect explicit language override in user message
 * @param {string} text - User's message
 * @returns {string|null} - Language code if override detected, null otherwise
 */
const detectLanguageOverride = (text) => {
  // Check for explicit language requests
  const arabicRequest =
    /(respond|answer|reply|talk|speak) in arabic|رد بالعربية|أجب بالعربية|تحدث بالعربية/i;
  const englishRequest =
    /(respond|answer|reply|talk|speak) in english|رد بالإنجليزية|أجب بالإنجليزية|تحدث بالإنجليزية/i;

  if (arabicRequest.test(text)) return "AR";
 ...
 * @param {string} userLanguage - Detected or requested language ('AR' or 'EN')
...
const getChatResponse = async (
  ...
  try {
    ...
    const languageInstructions = {
      AR: `\n\nCRITICAL LANGUAGE REQUIREMENT: You MUST respond EXCLUSIVELY in Arabic (العربية). Every single word in your response must be in Arabic. Do NOT use English words or mix languages. This is mandatory.`,
      EN: "", // No additional instruction for English
    };

    // Build context-aware, language-smart prompt
        ...
        parts: [
          ...
          ...(userGuideFile
            ? [
                {
                  fileData: {
                    fileUri: userGuideFile.uri,
                    mimeType: userGuideFile.mimeType,
                  },
                },
              ]
            : []),
...
export const sendMessage = async (req, res) => {
  try {
    ...
    // First check for explicit language override, then auto-detect
    const languageOverride = detectLanguageOverride(message);
    const detectedLanguage = languageOverride || detectLanguage(message);
