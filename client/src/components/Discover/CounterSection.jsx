...
function HorizontalStatCard({ endValue, label, description, icon, startAnimation, delay, format = 'number' }) {
    const count = useCountUp(endValue, 2000, startAnimation);

    let formattedValue = (format === 'number')
        ? new Intl.NumberFormat('en-US').format(count)
        : new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(count);
...
function AngledStatCard({ endValue, label, description, icon, startAnimation, delay, format = 'number', primaryGradient = false }) {
    const count = useCountUp(endValue, 2000, startAnimation);
    ...
    return (
        <div
            className={`
                ...
                ${primaryGradient ? 'bg-gradient-to-br from-green-600 to-green-800 text-white shadow-lg' : 'bg-gray-100 text-gray-900 shadow-md'}
            `}
            style={{ transitionDelay: `${delay}ms`, transform: 'skewX(-6deg)', transformOrigin: 'top left' }}
        >
            <div style={{ transform: 'skewX(6deg)' }}>
                <div className={`p-3 rounded-full mb-4 inline-block ${primaryGradient ? 'bg-green-800/50' : 'bg-white'}`}>
                    {icon}
                </div>
...
const HorizontalStatSkeleton = () => (
    <div className="rounded-2xl bg-gray-200 p-6 h-28 w-full animate-pulse">
        <div className="flex items-center gap-4">
            <div className="h-8 w-8 rounded-full bg-gray-300"></div>
            <div>
                <div className="h-10 w-24 rounded-md bg-gray-300"></div>
                <div className="h-4 w-20 mt-2 rounded-md bg-gray-300"></div>
            </div>
        </div>
    </div>
);
...
const AngledStatSkeleton = () => (
    <div className="rounded-2xl bg-gray-200 p-6 h-60 w-full animate-pulse" style={{ transform: 'skewX(-6deg)' }}>
        <div className="h-12 w-12 rounded-full bg-gray-300"></div>
        <div className="h-12 w-2/3 mt-4 rounded-md bg-gray-300"></div>
        <div className="h-6 w-1/2 mt-3 rounded-md bg-gray-300"></div>
        <div className="h-4 w-full mt-2 rounded-md bg-gray-300"></div>
    </div>
);
...
function CounterSection() {
    ...
    if (loading) {
        return (
            <section className="bg-green-100 rounded-lg py-16 sm:py-24">
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 gap-40 lg:grid-cols-2 lg:items-start">
                    ...
                    <div className="flex flex-col">
                        <div className="mx-auto mb-12 max-w-2xl w-full">
                            ...
                            <div className="mt-5 w-20 h-1 bg-green-700"></div>
                        </div>
